...
> **Want it online quickly?** See [DEPLOY-RAILWAY.md](DEPLOY-RAILWAY.md) for a host with a persistent disk (Dockerfile, volume and variables are already included), or [DEPLOY-SNAPDEPLOY.md](DEPLOY-SNAPDEPLOY.md) for a host with an **ephemeral** filesystem, which pairs the same image with a free hosted database (Turso) and free object storage (Cloudflare R2) so data survives restarts.
...
**Moving to object storage (S3, R2, MinIO, …):** set `STORAGE_DRIVER=s3` plus `S3_BUCKET`, `S3_ENDPOINT`, `S3_REGION`, `S3_ACCESS_KEY_ID` and `S3_SECRET_ACCESS_KEY` (see `.env.example`) - already implemented in `server/src/storage/S3Storage.ts`. The database stores opaque keys only, so nothing else changes. Useful on any host whose local disk doesn't persist (see DEPLOY-SNAPDEPLOY.md).

**Moving to a remote database (Turso, or any other hosted libSQL):** set `DATABASE_URL` to the `libsql://...` URL and `DATABASE_AUTH_TOKEN` to its token (see `.env.example`) - no schema or code changes needed, since the app already speaks the libSQL protocol. Migrations are applied automatically on every start whether the database is a local file or remote (see DEPLOY-SNAPDEPLOY.md).
...
| `npm run db:deploy:remote` | apply migrations to a remote database (`DATABASE_URL=libsql://...`) - used automatically by `scripts/start-prod.sh` when `DATABASE_URL` is remote |
